			...
			<h3 class="mt-3 mb-3">Libros</h3>
				<table class="table table-light table-striped " style="text-align: left;">
					...
					<%@ page import="java.sql.*"%>
						...
						<%
						String where = "INNER JOIN autores ON libros.idAutorFK=autores.idAutor INNER JOIN editoriales ON libros.idEditorialFK = editoriales.idEditorial";
						...
						<tr>
							...
							<td><%=resultSet.getString("tituloLibro")%></td>
							<td><%=resultSet.getString("precioLibro")%></td>
							<td><%=resultSet.getString("cantidadStockLibro")%></td>
							<td><%=resultSet.getString("autores.nombreAutor")%> <%=resultSet.getString("autores.apellidosAutor")%></td>
							<td><%=resultSet.getString("editoriales.nombreEditorial")%></td>
							<td><a class="btn btn-info" href="modificar.jsp?idLibro=<%=resultSet.getString("idLibro")%>">Modificar</a></td>
						</tr>
						<%
							}
						%>
